...
import { UserModel, type IUser } from "../db/models/Models";
import { authenticateJWT, generateToken, passport } from "../settings/passport";
...
userRouter.post("/", async (c) => {
    try {
        ...
        console.log("hashed pass -", userWithHashedPass)
...
userRouter.put("/:id", authenticateJWT, roleMiddleware('admin'), async (c) => {
    try {
        const { id } = c.req.param();
        const body = await c.req.json();
